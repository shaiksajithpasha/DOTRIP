...
export default function OutstationKmTab({ vendorId }: { vendorId: string }) {
  ...
  (async () => {
    try {
      ...
      // NEW: fetch saved limits
      const { limits } = await listOutstation(vendorId);
      if (!alive) return;

      const mapped = (limits || []).map((r: any) => ({
        id: String(r.id ?? `${r.vehicleTypeId}-${Math.random()}`),
        vehicleType: vtName(r.vehicleTypeId) || '',
        title: r.title ?? '',
        kmLimit: r.km ?? '',
      }));
      setItems(mapped);
    } catch (e) {
      console.error('outstation limits load failed', e);
      setItems([]);
    }
  })();
  return () => { alive = false; };
